...
策略默认每个市场最多成功入场 1 次，单次预算 `$5`，入场窗口为开盘后 `[20s, 250s]`，最后 40 秒只管理已有仓位。
...
- 普通 30% 止盈使用 GTC 被动卖单。
- 最后 40 秒内，`bid >= 0.9` 的直接止盈也使用 GTC 被动卖单。
...
| 参数 | 作用 | 默认 |
| --- | --- | --- |
| `GME_ORDER_USD` | 单次市场预算 | `5` |
| `GME_MAX_ENTRIES_PER_MARKET` | 单市场最大成功入场次数 | `1` |
| `GME_NO_ENTRY_FIRST_SECONDS` | 前多少秒不入场 | `20` |
...
| `GME_TAKE_PROFIT_ORDER_TYPE` | 普通止盈订单类型 | `GTC` |
| `GME_FINAL_DIRECT_TP_ORDER_TYPE` | 最后 40 秒直接止盈订单类型 | `GTC` |
