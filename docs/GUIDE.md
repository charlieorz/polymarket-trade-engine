...
| Gap Momentum Edge | `gap-momentum-edge` | `engine/strategy/gap-momentum-edge.ts` | BTC 5m simulation-first strategy that buys only the current gap-advantage side, uses fixed 6-share short-TTL GTC entries, FOK profit exits, and never sells a losing position. |
