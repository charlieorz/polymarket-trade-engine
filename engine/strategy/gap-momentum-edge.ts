...
export function readGapMomentumEdgeConfig(
  ...
  return {
    ...
    maxEntryPrice: clamp(parseNumberEnv(env, "GME_MAX_ENTRY_PRICE", 0.64), 0.01, 0.99),
    maxSpread: Math.max(0, parseNumberEnv(env, "GME_MAX_SPREAD", 0.04)),
    minEntryLiquidityUsd: Math.max(0, parseNumberEnv(env, "GME_MIN_ENTRY_LIQUIDITY_USD", 5)),
    minExitLiquidityUsd: Math.max(0, parseNumberEnv(env, "GME_MIN_EXIT_LIQUIDITY_USD", 5)),
    minAbsGap: Math.max(0, parseNumberEnv(env, "GME_MIN_ABS_GAP", 2)),
    minGapAtr: Math.max(0, parseNumberEnv(env, "GME_MIN_GAP_ATR", 0.5)),
    earlyMinGapAtr: Math.max(0, parseNumberEnv(env, "GME_EARLY_MIN_GAP_ATR", 0.5)),
    earlyRemainingSeconds: Math.max(
      0,
      parseNumberEnv(env, "GME_EARLY_REMAINING_SECONDS", 120),
    ),
    lateMinGapAtr: Math.max(0, parseNumberEnv(env, "GME_LATE_MIN_GAP_ATR", 0.35)),
    lateRemainingSeconds: Math.max(
      0,
      parseNumberEnv(env, "GME_LATE_REMAINING_SECONDS", 60),
    ),
    minPeakRetainRatio: clamp(parseNumberEnv(env, "GME_MIN_PEAK_RETAIN_RATIO", 0.35), 0, 1),
    minTrendConsistency: clamp(parseNumberEnv(env, "GME_MIN_TREND_CONSISTENCY", 0.25), 0, 1),
    minSideVelocityEma: parseNumberEnv(env, "GME_MIN_SIDE_VELOCITY_EMA", -0.35),
    minCumulativeGap: Math.max(0, parseNumberEnv(env, "GME_MIN_CUMULATIVE_GAP", 0)),
    minNetEdge: Math.max(0, parseNumberEnv(env, "GME_MIN_NET_EDGE", 0)),
    ...
    finalTakeProfitRatio: clamp(
      parseNumberEnv(
        ...
        parseNumberEnv(env, "GME_FINAL_DIRECT_TP_BID", 0.7),
      ),
      0.01,
      0.99,
    ),
    finalStopLossRatio: clamp(
      parseNumberEnv(env, "GME_FINAL_STOP_LOSS_RATIO", 0.9),
      0.01,
      0.99,
    ),
  };
