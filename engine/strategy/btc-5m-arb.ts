...
export function readBtc5mArbConfig(
  ...
  return {
    ...
    managedExitStartElapsedSeconds: Math.max(
      0,
      parseNumberEnv(env, "B5A_MANAGED_EXIT_START_SECONDS", 200),
    ),
    ...
    maxSpread: Math.max(0, parseNumberEnv(env, "B5A_MAX_SPREAD", 0.08)),
    ...
    fullTakeProfitRatio: Math.max(0, parseNumberEnv(env, "B5A_FULL_TAKE_PROFIT_RATIO", 0.45)),
    halfStopLossRatio: clamp(parseNumberEnv(env, "B5A_HALF_STOP_LOSS_RATIO", 0.3), 0, 0.99),
    fullStopLossRatio: clamp(parseNumberEnv(env, "B5A_FULL_STOP_LOSS_RATIO", 0.45), 0, 0.99),
