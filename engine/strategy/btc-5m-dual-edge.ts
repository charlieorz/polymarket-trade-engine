// BTC 5m dual-edge arbitrage strategy.
...
function parseSmallProfitExitModeEnv(
  ...
  const value = env.B5D_SMALL_PROFIT_EXIT_MODE?.trim().toLowerCase();
  if (
...
export function readBtc5mDualEdgeConfig(
  env: Record<string, string | undefined> = process.env,
): Config {
  return {
    tickIntervalMs: Math.max(50, parseNumberEnv(env, "B5D_TICK_INTERVAL_MS", 200)),
    statsIntervalMs: Math.max(250, parseNumberEnv(env, "B5D_STATS_INTERVAL_MS", 1000)),
    velocityEmaPeriod: Math.max(2, parseNumberEnv(env, "B5D_VELOCITY_EMA_PERIOD", 5)),
    trendLookback: Math.max(3, Math.floor(parseNumberEnv(env, "B5D_TREND_LOOKBACK", 8))),
    shares: Math.max(0.01, parseNumberEnv(env, "B5D_SHARES", 6)),
    entryStartElapsedSeconds: Math.max(0, parseNumberEnv(env, "B5D_ENTRY_START_SECONDS", 67)),
    entryEndElapsedSeconds: Math.max(0, parseNumberEnv(env, "B5D_ENTRY_END_SECONDS", 217)),
    managedExitStartElapsedSeconds: Math.max(
      0,
      parseNumberEnv(env, "B5D_MANAGED_EXIT_START_SECONDS", 200),
    ),
    holdOnlyStartElapsedSeconds: Math.max(
      0,
      parseNumberEnv(env, "B5D_HOLD_ONLY_START_SECONDS", 297),
    ),
    entryOrderType: parseOrderTypeEnv(env, "B5D_ENTRY_ORDER_TYPE", "FAK"),
    entryOrderTtlMs: Math.max(250, parseNumberEnv(env, "B5D_ENTRY_ORDER_TTL_MS", 750)),
    takeProfitOrderType: parseOrderTypeEnv(env, "B5D_TAKE_PROFIT_ORDER_TYPE", "GTC"),
    takeProfitOrderTtlMs: Math.max(
      250,
      parseNumberEnv(env, "B5D_TAKE_PROFIT_ORDER_TTL_MS", 3000),
    ),
    stopLossOrderType: parseOrderTypeEnv(env, "B5D_STOP_LOSS_ORDER_TYPE", "FAK"),
    stopLossOrderTtlMs: Math.max(
      250,
      parseNumberEnv(env, "B5D_STOP_LOSS_ORDER_TTL_MS", 1200),
    ),
    maxSpread: Math.max(0, parseNumberEnv(env, "B5D_MAX_SPREAD", 0.08)),
    minEntryLiquidityUsd: Math.max(0, parseNumberEnv(env, "B5D_MIN_ENTRY_LIQUIDITY_USD", 2.5)),
    minExitLiquidityUsd: Math.max(0, parseNumberEnv(env, "B5D_MIN_EXIT_LIQUIDITY_USD", 2.5)),
    maxAdvantagePrice: clamp(parseNumberEnv(env, "B5D_MAX_ADVANTAGE_PRICE", 0.6), 0.01, 0.99),
    maxReversalPrice: clamp(parseNumberEnv(env, "B5D_MAX_REVERSAL_PRICE", 0.52), 0.01, 0.99),
    enableAdvantage: parseBooleanEnv(env, "B5D_ENABLE_ADVANTAGE", true),
    enableReversal: parseBooleanEnv(env, "B5D_ENABLE_REVERSAL", true),
    advantageMinAbsGap: Math.max(0, parseNumberEnv(env, "B5D_ADV_MIN_ABS_GAP", 1.5)),
    advantageMinMomentum: Math.max(0, parseNumberEnv(env, "B5D_ADV_MIN_MOMENTUM", 0.05)),
    advantageMinCumulativeGap: Math.max(
      0,
      parseNumberEnv(env, "B5D_ADV_MIN_CUMULATIVE_GAP", 2),
    ),
    reversalMaxAbsGap: Math.max(0, parseNumberEnv(env, "B5D_REV_MAX_ABS_GAP", 8)),
    reversalMinMomentum: Math.max(0, parseNumberEnv(env, "B5D_REV_MIN_MOMENTUM", 0.08)),
    reversalMinCumulativeGap: Math.max(
      0,
      parseNumberEnv(env, "B5D_REV_MIN_CUMULATIVE_GAP", 6),
    ),
    minTakeProfitRatio: Math.max(0.18, parseNumberEnv(env, "B5D_MIN_TAKE_PROFIT_RATIO", 0.24)),
    maxTakeProfitRatio: Math.max(0.12, parseNumberEnv(env, "B5D_MAX_TAKE_PROFIT_RATIO", 0.44)),
    takeProfitPriceImmediate: clamp(
      parseNumberEnv(env, "B5D_TAKE_PROFIT_PRICE_IMMEDIATE", 0.87),
      0.01,
      0.99,
    ),
    fullTakeProfitRatio: Math.max(0, parseNumberEnv(env, "B5D_FULL_TAKE_PROFIT_RATIO", 0.45)),
    halfStopLossRatio: clamp(parseNumberEnv(env, "B5D_HALF_STOP_LOSS_RATIO", 0.3), 0, 0.99),
    fullStopLossRatio: clamp(parseNumberEnv(env, "B5D_FULL_STOP_LOSS_RATIO", 0.45), 0, 0.99),
    stopLossStartElapsedSeconds: Math.max(
      0,
      parseNumberEnv(env, "B5D_STOP_LOSS_START_SECONDS", 0),
    ),
    stopLossMinHoldSeconds: Math.max(0, parseNumberEnv(env, "B5D_STOP_LOSS_MIN_HOLD_SECONDS", 0)),
    entryTakeProfitEnabled: parseBooleanEnv(env, "B5D_ENTRY_TAKE_PROFIT_ENABLED", false),
    managedTakeProfitEnabled: parseBooleanEnv(env, "B5D_MANAGED_TAKE_PROFIT_ENABLED", true),
    stopLossEnabled: parseBooleanEnv(env, "B5D_STOP_LOSS_ENABLED", true),
    smallProfitExitMode: parseSmallProfitExitModeEnv(env),
    halfStopHoldRestToSettlement: parseBooleanEnv(
      env,
      "B5D_HALF_STOP_HOLD_REST_TO_SETTLEMENT",
      false,
    ),
    dynamicTpPriceWeight: Math.max(0, parseNumberEnv(env, "B5D_DYNAMIC_TP_PRICE_WEIGHT", 0.1)),
    dynamicTpGapWeight: Math.max(0, parseNumberEnv(env, "B5D_DYNAMIC_TP_GAP_WEIGHT", 0.08)),
    dynamicTpMomentumWeight: Math.max(
      0,
      parseNumberEnv(env, "B5D_DYNAMIC_TP_MOMENTUM_WEIGHT", 0.06),
    ),
  };
}

const CONFIG = readBtc5mDualEdgeConfig();
...
function placeSell(params: {
  ...
  const signalId = params.ctx.recordSignal({
    ...
    label: `btc-5m-dual-edge ${params.decision.reason}`,
    metrics: sellMetrics,
  });
  ...
  params.ctx.postOrders([
    {
      ...
      onFilled(filledShares) {
        ...
        params.ctx.log(
          `[${params.ctx.slug}] btc-5m-dual-edge: SELL ${params.pos.side} filled @ ${params.decision.price} (${filledShares} shares, ${params.decision.reason})`,
          "green",
        );
      ...
      onExpired() {
        params.state.closing = false;
        params.ctx.log(
          `[${params.ctx.slug}] btc-5m-dual-edge: SELL ${params.pos.side} expired (${params.decision.reason})`,
          "yellow",
        );
      },
      onFailed(reason) {
        params.state.closing = false;
        params.ctx.log(
          `[${params.ctx.slug}] btc-5m-dual-edge: SELL ${params.pos.side} failed (${reason})`,
          "red",
        );
      },
    },
...
export const btc5mDualEdge: Strategy = async (ctx) => {
  if (Env.get("PROD") && !parseBooleanEnv(process.env, "B5D_ALLOW_PROD", false)) {
    ctx.log(
      "[btc-5m-dual-edge] Strategy is simulation-first by default. Set B5D_ALLOW_PROD=true only after fresh replay and small-size paper runs are stable.",
      "red",
    );
    process.exit(1);
  }
  ...
  if (Env.get("MARKET_ASSET") !== "btc" || Env.get("MARKET_WINDOW") !== "5m") {
    ctx.log("[btc-5m-dual-edge] Strategy only supports BTC 5m markets.", "yellow");
    return;
  }
  ...
  const interval = setInterval(() => {
    ...
    if (entry) {
      ...
      const signalId = ctx.recordSignal({
        action: "buy",
        side: entry.side,
        label: `btc-5m-dual-edge ${entry.kind} entry`,
        metrics: entryMetrics,
      });
      state.entryOrderSubmitted = true;
      state.pendingEntry = true;
      ctx.log(
        `[${ctx.slug}] btc-5m-dual-edge: signal BUY ${entry.kind} ${entry.side} @ ${entry.price} tp ${entry.takeProfitPrice}`,
        "cyan",
      );
      ctx.postOrders([
        {
          ...
          onFilled(filledShares) {
            ...
            ctx.log(
              `[${ctx.slug}] btc-5m-dual-edge: BUY ${entry.kind} ${entry.side} filled @ ${entry.price} (${filledShares} shares)`,
              "green",
            );
          },
          onExpired() {
            state.pendingEntry = false;
            ctx.log(
              `[${ctx.slug}] btc-5m-dual-edge: BUY ${entry.kind} ${entry.side} expired @ ${entry.price}`,
              "yellow",
            );
          ...
          onFailed(reason) {
            state.pendingEntry = false;
            ctx.log(
              `[${ctx.slug}] btc-5m-dual-edge: BUY ${entry.kind} ${entry.side} failed (${reason})`,
              "red",
            );
            if (!state.position) releaseOnce(state, release);
          },
    ...
    if (elapsed >= CONFIG.holdOnlyStartElapsedSeconds) {
      if (!state.settlementHoldLogged) {
        ...
        ctx.log(
          `[${ctx.slug}] btc-5m-dual-edge: holding ${pos.side} to settlement inside final hold-only window`,
          "cyan",
        );
      }
      return;
...
export const __btc5mDualEdgeTestHooks = {
  readBtc5mDualEdgeConfig,
  createEdgeStats,
  updateStats,
  chooseEntry,
  chooseExit,
