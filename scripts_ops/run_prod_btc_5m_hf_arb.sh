...
if [[ -z "${PRIVATE_KEY:-}" ]]; then
  echo "PRIVATE_KEY is required for production trading." >&2
  exit 2
fi

if [[ -z "${POLY_FUNDER_ADDRESS:-}" ]]; then
  echo "POLY_FUNDER_ADDRESS is required for production trading." >&2
  exit 2
fi
...
# Keep Env.get("PROD") true so btc-5m-hf-arb can enforce B5H_ALLOW_PROD.
# The --prod CLI flag below makes EarlyBird use the real CLOB client.
export PROD=true
export FORCE_PROD=true
export POLY_STRATEGY=btc-5m-hf-arb
