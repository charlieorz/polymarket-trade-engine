...
function advantageStats() {
  return {
    ...__btc5mDualEdgeTestHooks.createEdgeStats(),
...
describe("btc-5m-dual-edge", () => {
  test("parses the requested timing and order-type defaults", () => {
    const config = __btc5mDualEdgeTestHooks.readBtc5mDualEdgeConfig({});
    ...
    expect(config.reversalMinCumulativeGap).toBe(6);
  ...
  test("chooses an advantage entry only inside the 67-217 second window", () => {
    const config = __btc5mDualEdgeTestHooks.readBtc5mDualEdgeConfig({
      B5D_ADV_MIN_ABS_GAP: "4",
      B5D_ADV_MIN_MOMENTUM: "0.2",
      B5D_ADV_MIN_CUMULATIVE_GAP: "30",
    });
    ...
    expect(
      __btc5mDualEdgeTestHooks.chooseEntry({
        ctx: mockCtx(),
        gap: 10,
        elapsed: 66,
        stats: advantageStats(),
  ...
  test("chooses a reversal entry below the reversal price cap", () => {
    const entry = __btc5mDualEdgeTestHooks.chooseEntry({
      ...
      stats: reversalStats(),
      state: baseState(),
      config: __btc5mDualEdgeTestHooks.readBtc5mDualEdgeConfig({
        B5D_ENABLE_REVERSAL: "true",
        B5D_REV_MAX_ABS_GAP: "4",
        B5D_REV_MIN_MOMENTUM: "0.2",
      }),
    });
  ...
  test("uses dynamic take-profit with a higher minimum entry-window ratio", () => {
    const ratio = __btc5mDualEdgeTestHooks.dynamicTakeProfitRatio({
      kind: "advantage",
      price: 0.64,
      absGap: 4,
      momentum: 0.18,
  ...
  test("takes entry-window take-profit and allows enabled entry-window stop-loss", () => {
    const tp = __btc5mDualEdgeTestHooks.chooseExit({
      ...
      elapsed: 180,
      config: __btc5mDualEdgeTestHooks.readBtc5mDualEdgeConfig({
        B5D_ENTRY_TAKE_PROFIT_ENABLED: "true",
      }),
    });
    expect(tp?.reason).toBe("dynamic take-profit");
    expect(tp?.orderType).toBe("GTC");

    const disabledStop = __btc5mDualEdgeTestHooks.chooseExit({
    ...
    const enabledStop = __btc5mDualEdgeTestHooks.chooseExit({
      ctx: mockCtx({ upBid: 0.2, upAsk: 0.22 }),
      pos: { ...basePosition },
      gap: -20,
      ask: 0.22,
  ...
  test("applies managed take-profit priority in the 222-297 second window", () => {
    const priceTp = __btc5mDualEdgeTestHooks.chooseExit({
    ...
    const fullTp = __btc5mDualEdgeTestHooks.chooseExit({
    ...
    const costCover = __btc5mDualEdgeTestHooks.chooseExit({
    ...
    const legacyCostCover = __btc5mDualEdgeTestHooks.chooseExit({
      ctx: mockCtx({ upBid: 0.62, upAsk: 0.64 }),
      pos: { ...basePosition },
      gap: 10,
      ask: 0.64,
      ...
      elapsed: 230,
      config: __btc5mDualEdgeTestHooks.readBtc5mDualEdgeConfig({
        B5D_SMALL_PROFIT_EXIT_MODE: "cost_cover_hold",
      }),
    });
  ...
  test("uses FAK for managed half and full stop-loss exits", () => {
    const half = __btc5mDualEdgeTestHooks.chooseExit({
    ...
    const legacyHalf = __btc5mDualEdgeTestHooks.chooseExit({
      ctx: mockCtx({ upBid: 0.27, upAsk: 0.29 }),
      pos: { ...basePosition },
      gap: -12,
      ask: 0.29,
